...
def extract_time_series(station_name, fuel_type):
	...
	for fileName in fileList:
		with open('.\\csv\\'+fileName) as f:
			for line in f:
				if StationName in line and fuelType in line:
					keyItems = line.split(',')
					#print(keyItems[7])
	...
	if len(dateTimeList) != 0:
		...
		counter = 0
		with open('.\\station_fuelType\\'+StationName+'_'+fuelType, "w") as output_file:
